...
# 创建数据表
create_table_sql = """
CREATE TABLE IF NOT EXISTS gt42_homologous_id (
    id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    genomeID VARCHAR(255) NOT NULL UNIQUE,
    homologousIDSet LONGTEXT NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
"""
cursor.execute(create_table_sql)

# 为genomeID列创建索引以加快查询速度
cursor.execute("ALTER TABLE gt42_homologous_id ADD INDEX idx_genomeID (genomeID);")
...
#         insert_sql = "INSERT INTO gt42_homologous_id (genomeID, type) VALUES (%s, %s);"
...
# 读取数据文件并插入数据
data_file = './table_for_mysql/homologous_id_table.tsv'
with open(data_file, newline='', encoding='utf-8') as csvfile:
    reader = csv.reader(csvfile, delimiter='\t') # 使用制表符分隔
    next(reader)  # 跳过标题行
    ...
    for line in reader:
        if len(line) == 2:
            ...
            if len(batch_data) >= batch_size:
                cursor.executemany("INSERT INTO gt42_homologous_id (genomeID, homologousIDSet) VALUES (%s, %s);", batch_data)
                batch_data = []  # 清空批处理列表
            
            cnt += 1
            if cnt % 10000 == 0: # 约51万行，共耗时700s
                print(f"{cnt} records inserted. Time elapsed: {time.time() - start_time:.2f} seconds.")
