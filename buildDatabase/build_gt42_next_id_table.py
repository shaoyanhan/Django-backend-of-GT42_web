...
# 创建数据表
create_table_sql = """
CREATE TABLE IF NOT EXISTS gt42_next_id (
    id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    nextID VARCHAR(255) NOT NULL UNIQUE,
...
# 读取数据文件并插入数据
data_file = './table_for_mysql/gt42_next_id_table.tsv'
with open(data_file, newline='', encoding='utf-8') as csvfile:
    reader = csv.reader(csvfile, delimiter='\t') # 使用制表符分隔
    next(reader)  # 跳过标题行
    ...
    for line in reader:
        if len(line) == 2:
            ...
            if len(batch_data) >= batch_size:
                cursor.executemany("INSERT INTO gt42_next_id (nextID, type) VALUES (%s, %s);", batch_data)
                batch_data = []  # 清空批处理列表
